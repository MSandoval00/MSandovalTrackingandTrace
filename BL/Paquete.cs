            ...
            try
            {
                using (DL.MSandovalTrackingandTraceEntities context=new DL.MSandovalTrackingandTraceEntities())
                {
                        ...
                        var query = (from tablaPaquete in context.Paquetes
                                     ...
                                     where (usuario.Nombre==null || tablaUsuario.Nombre.Contains(usuario.Nombre))&& (estatusEntrega.IdEstatus==0 || tablaEstatusEntrega.IdEstatus == estatusEntrega.IdEstatus)
                                     select new
                                     {
                                         //Paquete
